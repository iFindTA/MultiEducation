...
@interface MEWatchHistoryProfile () <DZNEmptyDataSetSource, DZNEmptyDataSetDelegate, UITableViewDelegate, UITableViewDataSource>
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if (self.dataSource.count == 0) {
        ...
        [self autoLoadMoreHistoryWatchingItems4PageIndex:1];
    }
}
...
/**
 加载数据
 */
- (void)autoLoadMoreHistoryWatchingItems4PageIndex:(NSUInteger)index {
    ...
    [vm postData:[res data] pageSize:ME_PAGING_SIZE pageIndex:index hudEnable:true success:^(NSData * _Nullable resObj, NSUInteger totalPages) {
        ...
        if (err) {
            ...
            NSLog(@"total history pages:%zd", totalPages);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    static NSString *identifier = @"user_history_story_item_cell";
    ...
    cell.indexContentItemCallback = ^(NSUInteger section, NSUInteger index){
        strongify(self)
        [self userWathingHistoryStoryItemDidTouchRowIndex:index];
    };
...
- (void)userWathingHistoryStoryItemDidTouchRowIndex:(NSUInteger)index {
